...
/// [SearchDelegate1.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [SearchDelegate1] can only be associated with one active [showSearch1]
/// call. Call [SearchDelegate1.close] before re-using the same delegate instance
/// for another [showSearch1] call.
///
...
/// The animation provided by [SearchDelegate1.transitionAnimation] can be used
...
/// See also:
///
///  * [SearchDelegate1] to define the content of the search page.
///
///  MWANZO WA COMMENT YANGU
// Future<T> showSearch1<T>({
//   @required BuildContext context,
//   @required SearchDelegate1<T> delegate,
//   String query = '',
// }) {
...
//   ));
// }
//MWISHO WA COMMENT YANGU NA MWANZO WA GPT
...
Future<T> showSearch1<T>({
  ...
  delegate.query = query ?? delegate.query;
  delegate._currentBody = _SearchBody.suggestions;
  return await Navigator.of(context).push<T>(_SearchPageRoute<T>(
    delegate: delegate,
  ));
}
// MWISHO WA GPT
...
/// Delegate for [showSearch1] to define the content of the search page.
///
/// The search page always shows an [AppBar] at the top where users can
/// enter their search queries. The buttons shown before and after the search
/// query text field can be customized via [SearchDelegate1.leading] and
/// [SearchDelegate1.actions].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [SearchDelegate1.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [SearchDelegate1.buildResults].
///
/// [SearchDelegate1.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [SearchDelegate1.showResults]
/// and you can go back to showing the suggestions by calling
/// [SearchDelegate1.showSuggestions].
///
/// Once the user has selected a search result, [SearchDelegate1.close] should be
/// called to remove the search page from the top of the navigation stack and
/// to notify the caller of [showSearch1] about the selected search result.
///
/// A given [SearchDelegate1] can only be associated with one active [showSearch1]
/// call. Call [SearchDelegate1.close] before re-using the same delegate instance
/// for another [showSearch1] call.
abstract class SearchDelegate1<T> {
  ...
  /// communicate the result back to the initial caller of [showSearch1].
...
//    _currentBody = _SearchBody.results;
  ...
  /// to [showSearch1] that launched the search initially.
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [SearchDelegate1.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [SearchDelegate1.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  ...
  final SearchDelegate1<T> delegate;
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          ...
          return AlertDialog(
            ...
            content: new Text("Search cannot be empty"),
            actions: <Widget>[
        ...
        appBar: AppBar(
          ...
          title: TextField(
            ...
            onSubmitted: (String _) {
              if (widget.delegate.query == '') {
                print('widget.delegate.query');
                _showDialog();
              } else {
